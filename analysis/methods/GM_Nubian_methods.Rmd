...
**Data:** Corresponding .ply and .pts files for 115 cores from NK sites and 51 from TH sites (total, 166) in folder "Core_data".\

```{r include=FALSE}
knitr::opts_knit$set(root.dir = "C:/Users/eshal/Dropbox/R/nubianGM-master/GM_METHODS")
```
...
```{r Step 5.1}
## Ensure GM_METHODS folder is set as the WD
# Define a path to the folder with core data
core_data <- file.path("Core_data")
...
save(core_lms, file = "core_lms.RData")
...
save(alloutline, file = "alloutline.RData")
```
...
save(allpatched, file = "allpatched.RData")
```
...
```{r Step 6.1}
# Define a path to the folder with core data
Product_data <- file.path("Product_data")
...
save(prod_ventr, file = "prod_ventr.RData")
```
...
cores <- read.csv("File_list_cores.csv", header=TRUE, stringsAsFactors = TRUE)
```
